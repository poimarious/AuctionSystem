...
public class BidHistoryController {
  ...
  public void initialize() {
    ...
    // 2. Cột Giá bạn đặt
    myBidPriceColumn.setCellValueFactory(cellData ->
    ...
    currentPriceColumn.setCellValueFactory(cellData -> {
      if (cellData.getValue().getAuction() != null) {
        return new SimpleStringProperty(String.format("$%.2f", cellData.getValue().getAuction().getCurrentPrice()));
      }
    ...
    statusColumn.setCellValueFactory(cellData -> {
      if (cellData.getValue().getAuction() != null) {
        ...
        return new SimpleStringProperty(cellData.getValue().getAuction().getStatus().toString());
      }
      return new SimpleStringProperty("N/A");
    });
  ...
  private void loadBidHistory() {
    ...
    if (currentUser instanceof Bidder) {
      ...
      if (bidder.getBidHistory() != null) {
        ...
        System.out.println("Đã tải thành công " + myBids.size() + " lượt đặt giá cho User: " + bidder.getUsername());
      } else {
