...
public class AddProductController {
    ...
    public void handleUploadProduct(ActionEvent event) {
        ...
        if (name == null || name.trim().isEmpty() || category == null ||
                ...
                description == null || description.trim().isEmpty() || endDate == null) {
        ...
        if (!(currentUser instanceof Seller)) {
            showAlert(Alert.AlertType.ERROR, "Từ chối truy cập", "Chỉ có Người bán mới được đăng sản phẩm!");
            return;
        }
        ...
        if (selectedFactory == null) {
            showAlert(Alert.AlertType.ERROR, "Lỗi hệ thống", "Không tìm thấy Factory xử lý cho danh mục này!");
            return;
        }
        ...
        if (newItem != null) {
            ...
            if (response != null && "SUCCESS".equals(response.getStatus())) {
                showAlert(Alert.AlertType.INFORMATION, "Thành công", "Sản phẩm và Hình ảnh đã được lưu trữ an toàn trên Server!");
                SceneManager.getInstance().goBack();
            } else {
